...
function normalizeLog(e: Record<string, unknown>): Record<string, unknown> {
  ...
  const identityKind = (e.identity_kind as string) ?? (identityValue?.includes('@') ? 'email' : 'phone');
  return {
    id: String(e.id ?? ''),
    type: channel,
...
export const GET: RequestHandler = async ({ url, locals }) => {
  ...
  if (!isA2pCommLogEnabled()) {
    return json({ error: 'A2P comm log not configured (set A2P_COMMLOG_API_URL or A2P_DATABASE_URL)' }, { status: 503 });
  }
  try {
