...
| `/api/push`                     | register POST, unregister DELETE                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
...
| POST   | `/api/push/register`   | Session | Body: `{ deviceId, platform?, fcmToken?, voipToken? }` | `{ success, device }` |
| DELETE | `/api/push/unregister` | Session | Body: `{ deviceId }` | `{ success, message }` |

### VoIP Push Sending (Backend Logic)

When a `call.initiated` or `call.hangup` event is received via Telnyx Webhook, the backend performs the following:

1.  **Lookup:** Find the user/company associated with the `to` phone number.
2.  **Dispatch:** Retrieve all `user_devices` for the associated user(s).
3.  **Transport:**
    *   **iOS:** Sends APNs VoIP Push (Topic: `{bundleId}.voip`, Priority: 10, Type: `voip`).
    *   **Android:** Sends FCM High-Priority Data message (No `notification` key).
4.  **Payload Type:** `incoming_call` (to ring) or `cancel_call` (to stop ringing).

*Note: Requires VoIP certificate for iOS and FCM Service Account for Android.*
