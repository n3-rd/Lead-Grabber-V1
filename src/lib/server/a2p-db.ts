...
export async function queryCommEvents(opts: QueryCommEventsOptions = {}): Promise<CommEventRow[]> {
  const { limit = 50, offset = 0, contactId, channel, direction, internalTsFrom, internalTsTo } = opts;
...
function inferIdentityKindValue(source: string | null | undefined): { kind: 'phone' | 'email'; value: string } | null {
  const s = (source ?? '').trim();
  if (!s) return null;
