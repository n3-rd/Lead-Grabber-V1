...
export async function forwardVoiceWebhook(rawBody: string): Promise<{ ok: boolean; status: number; body: unknown }> {
...
export async function forwardSmsWebhook(rawBody: string): Promise<{ ok: boolean; status: number; body: unknown }> {
...
export async function healthCheck(path: '/healthz' | '/health' = '/health'): Promise<{ ok: boolean; error?: string }> {
  try {
    const target = url(path);
    const res = await fetch(target);
    const data = res.ok ? await res.json().catch(() => ({})) : {};
    return res.ok ? { ok: (data as { ok?: boolean }).ok !== false } : { ok: false, error: (data as { error?: string }).error ?? `HTTP ${res.status}` };
  } catch (e) {
