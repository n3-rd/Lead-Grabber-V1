...
export const POST: RequestHandler = async ({ request }) => {
    try {
        ...
        if (!mappedPayload.to || !mappedPayload.from) {
            return json({ error: 'Invalid payload: "to" and "from" fields are required' }, { status: 400 })
        }

        const result = await processInboundEmail(mappedPayload)

        if (result.success) {
            return json({ message: 'Email processed successfully', id: result.messageId }, { status: 201 })
        } else {
            // We return 200/202 even on "Tenant not found" to avoid retries from the gateway
            // if it's a permanent error like "unknown user".
            return json({ error: result.error }, { status: 202 })
        }
    } catch (error) {
