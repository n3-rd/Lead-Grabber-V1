...
| Prefix | Endpoints |
|--------|-----------|
| `/api/auth` | signup, login, otp/send, otp/verify |
...
| Method | Path | Auth | Request | Response |
|--------|------|------|---------|----------|
| POST | `/api/email/send` | Session | Body: recipients[], subject, body, fromName? | `{ success, data: { results }, message }` (stub: logs to comm log) |
