...
export const GET: RequestHandler = async ({ url, locals }) => {
  ...
  try {
    ...
    return json({ error: e instanceof Error ? e.message : 'Failed to fetch notifications' }, { status: 500 })
  }
}
...
export const PATCH: RequestHandler = async ({ request, locals }) => {
  ...
  try {
    ...
    if (id == null || read === undefined) {
      return json({ error: 'id and read are required' }, { status: 400 })
    }
    ...
    return json({ error: e instanceof Error ? e.message : 'Failed to update notification' }, { status: 500 })
  }
}
