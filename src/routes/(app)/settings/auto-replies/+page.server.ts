...
export const load: PageServerLoad = async ({ locals }) => {
	...
	try {
		...
		const autoReply = (settings.autoReply && typeof settings.autoReply === 'object' && !Array.isArray(settings.autoReply)
			? settings.autoReply
			: defaultAutoReply) as typeof defaultAutoReply
	...
	saveAutoReply: async ({ request, locals }) => {
		...
		try {
			...
			settings.autoReply = {
				...
				businessHoursMessage: autoReplyData.businessHoursMessage ?? defaultAutoReply.businessHoursMessage,
				afterHoursMessage: autoReplyData.afterHoursMessage ?? defaultAutoReply.afterHoursMessage,
				leadformBusinessHoursMessage: autoReplyData.leadformBusinessHoursMessage ?? defaultAutoReply.leadformBusinessHoursMessage,
				leadformAfterHoursMessage: autoReplyData.leadformAfterHoursMessage ?? defaultAutoReply.leadformAfterHoursMessage,
