...
async function nextPersonalCode(companyId: string, userId: string): Promise<string> {
	...
	const nums = existing.map((s) => parseInt(s.code.replace(/^\//, ''), 10)).filter((n) => !Number.isNaN(n));
...
export const POST: RequestHandler = async ({ request, locals }) => {
	...
	if (!message) {
		return json({ success: false, error: 'message is required (max 255 chars)', code: 400 }, { status: 400 });
	}
	...
	return json(
		{ success: true, data: { id: shortcut.id, code: shortcut.code, message: shortcut.message }, message: 'Shortcut created successfully' },
		{ status: 201 }
	);
};
